import React from 'react';
import { 
  FaPython, 
  FaJava, 
  FaDocker, 
  FaGithub,
  FaAws,
  FaTerminal,
  FaCloud,
  FaMicrosoft
} from 'react-icons/fa';
import { 
  SiCplusplus, 
  SiGit, 
  SiKubernetes,
  SiFastapi,
  SiSpringboot,
  SiTwilio,
  SiAnthropic,
  SiLangchain,
  SiGooglecloud,
  SiRedhat,
  SiTensorflow,
  SiPytorch,
  SiPandas,
  SiNumpy,
  SiC,
  SiRedis,
  SiReact,
  SiPhp,
  SiTypescript,
  SiJupyter,
  SiOpenai,
  SiHuggingface,
  SiPostman,
  SiIntellijidea,
  SiSpring,
  SiNodedotjs,
  SiApache,
  SiElasticsearch,
  SiDocker
} from 'react-icons/si';
import { VscAzureDevops } from "react-icons/vsc";
import { PiFileSql } from "react-icons/pi";
import Card3D from './components/Card3D';
import './TiltCards.css';

const tiltCards = [
  {
    title: "FullStack Developer",
    icon: <SiReact />,
    color: "#667eea",
    description: "Building complete web applications from frontend to backend",
    svgPath: "/images/fullstack.svg"
  },
  {
    title: "Backend Developer",
    icon: <SiSpringboot />,
    color: "#764ba2",
    description: "Developing robust APIs and microservices architecture",
    svgPath: "/images/backend.svg"
  },
  {
    title: "Cloud Developer",
    icon: <FaAws />,
    color: "#f093fb",
    description: "Designing and managing cloud infrastructure solutions",
    svgPath: "/images/cloud.svg"
  },
  {
    title: "AI/ML Developer",
    icon: <SiTensorflow />,
    color: "#f5576c",
    description: "Building intelligent systems and machine learning models",
    svgPath: "/images/ai.svg"
  }
];

const TiltCards: React.FC = () => {
  return (
    <section className="tilt-cards-section">
      <div className="container">
        <div className="cards-perspective">
          <div className="tilt-cards-grid">
            {tiltCards.map((card, index) => (
              <Card3D
                key={index}
                title={card.title}
                content={card.description}
                color={card.color}
                svgPath={card.svgPath}
              />
            ))}
          </div>
        </div>
      </div>
    </section>
  );
};

export default TiltCards;
